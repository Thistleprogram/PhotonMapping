...
int render(const int width, const int height, const int samples, const int supersamples, const double d, Random *rnd) {
	// カメラ位置
				...
				for (int sx = 0; sx < supersamples; sx++) {
					...
					for (int s = 0; s < samples; s++) {
						...
						accumulated_radiance = accumulated_radiance +
							radiance(Ray(camera_position, dir), rnd, 0, 0, d) / samples / (supersamples * supersamples);
					}
